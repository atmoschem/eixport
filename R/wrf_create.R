...
wrf_create  <- function(wrfinput_dir         = "",
                        ...
                        kemit                = 1,
                        variaveis = c("E_NO2", "E_NO", "E_TOL",
                                      "E_XYL", "E_ALD", "E_ALDX",
                                      "E_SO2", "E_CO", "E_OLT",
                                      "E_OLI", "E_OL2", "E_NH3",
                                      "E_ISO", "E_HCL", "E_HCHO",
                                      "E_ETH", "E_CH3OH", "E_C2H5OH",
                                      "E_HC3", "E_HC5", "E_HC8",
                                      "E_KET", "E_ORA2", "E_CSL",
                                      "E_TERP", "E_PM25I","E_PM25J",
                                      "E_ECI", "E_ECJ", "E_ORGI",
                                      "E_ORGJ", "E_SO4I", "E_SO4J",
                                      "E_NO3J", "E_NO3I", "E_SO4C",
  ...
  for(domain in domains){
    ...
    if(frames_per_auxinput5 == 1){
      ...
      for(i in 1:frames_per_auxinput5){
        file_time[i] <- paste(format(date, "%Y-%m-%d"), "_", hora, ":", minuto,
                              ":", segundo, sep = "")
    ...
    west_east           <- ncdim_def("west_east", units = "",
                                     longname = "",
                                     vals = 1:g_atributos$`WEST-EAST_PATCH_END_UNSTAG`)
    south_north         <- ncdim_def("south_north",
                                     units = "",
                                     longname = "",
                                     vals = 1:g_atributos$`SOUTH-NORTH_PATCH_END_UNSTAG`)
    emissions_zdim_stag <- ncdim_def("emissions_zdim_stag", units = "",
                                     longname = "", vals = 1:kemit)
    DateStrLen          <- ncdim_def("DateStrLen", units = "",
                                     longname = "", vals = 1:19)
    Time                <- ncdim_def("Time", units = "", longname = "",
                                     vals = 1:frames_per_auxinput5, unlim = TRUE)
    # definition of variables
    Times               <- ncvar_def(name = "Times",
                                     dim = list(DateStrLen,Time),
                                     units = "",
                                     prec = "char",
                                     compression = COMPRESS)
    XLONG               <- ncvar_def(name = "XLONG",
                                     units = "",
                                     dim = list(west_east,south_north),
                                     prec = "float",
                                     compression = COMPRESS)
    XLAT                <- ncvar_def(name = "XLAT",
                                     units = "",
                                     dim = list(west_east,south_north),
                                     prec = "float",
                                     compression = COMPRESS)
    ...
    emiss_file <- nc_create(filename = file_name,
                            vars = c(list('Times' = Times, 'XLAT' = XLAT,
                                          'XLONG' = XLONG),
                                     mget(ls(pattern = "E_"))),
                            force_v4 = force_ncdf4)
    for(i in 1:length(g_atributos)){
      ncatt_put(emiss_file,
                varid = 0,
